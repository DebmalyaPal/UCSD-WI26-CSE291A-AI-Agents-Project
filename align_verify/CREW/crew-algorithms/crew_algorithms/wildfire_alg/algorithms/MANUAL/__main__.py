...
def wildfire_alg(cfg: Config):
    ...
    path = os.path.join(logs_root, "results", "logs", "MANUAL", level, str(seed), cfg.envs.timestamp)
    ...
    for t in range(cfg.envs.max_steps):
        ...
        action_tensor = torch.tensor(env_action, dtype=torch.float32, device=device)
