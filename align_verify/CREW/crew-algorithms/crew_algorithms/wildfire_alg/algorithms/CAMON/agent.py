...
class Agent:
    def __init__(self, id:int, type:int, cfg:Config, path, current_task:str, api_key) -> None:
        ...
        # Timing metadata for high-level options
        # option_start_time: timestep when the current head option was assigned
        # last_option_review_time: last timestep when feasibility of the current option was reviewed
        self.option_start_time = 0
        self.last_option_review_time = None

        # Cancellation metadata so the leader can be informed when an option
        # is stopped mid-execution.
        self.option_cancelled_recently = False
        self.last_cancelled_option = None
        # If True, force a no-op low-level action on the next env step
        # to reliably override any ongoing movement primitive.
        self.force_stop_next_step = False
    ...
    def log_chat(self, chat_name: str, messages: List[Tuple[str, str]]):
        ...
        # Ensure the per-agent log directory exists
    ...
    def generate_perception(self, cfg: Config, agent_states: Dict[int, Tuple[int, int]], global_data:dict):
        ...
        client = OpenAI(api_key=self.api_key, base_url="https://tritonai-api.ucsd.edu")
