...
class Config:
    ...
    enable_option_review: bool = False
    """Whether to enable per-option feasibility review before executing low-level actions."""
...
def wildfire_alg(cfg: Config):
    ...
    from crew_algorithms.wildfire_alg.algorithms.CAMON.utils import generate_plan, propose_plan, Action, review_option_feasibility
    ...
    print(f"[CAMON] Level: {level}, Seed: {seed}")
    print("[CAMON] Agent Count:", len(agents))
    ...
    print(f"[CAMON] Max Steps: {cfg.envs.max_steps}")

    # Two-phase cadence:
    # - Even timesteps (t=0,2,4,...): PLAN (perceive + assign options)
    # - Odd timesteps  (t=1,3,5,...): EXECUTE (emit low-level action + env.step)
    # This enforces: plan at t, execute at t+1, then plan at t+2.
    for t in range(cfg.envs.max_steps):
        print(f"\n[CAMON] ===== Time step {t} =====")

        # Always refresh lightweight agent observation fields for the current state
        # (positions, extra vars, etc.) so execution/primitives use up-to-date info.
        game_data = parse_game_data(state, cfg)
        removelist = []
        ...
        print(f"[CAMON] Active agents: {len(agents)}, Score: {global_data['score']}")
        ...
        for agent in agents:
            ...
            agent_data = {
                ...
                'last_cancelled_action': agent.last_cancelled_option
            }
        ...
        for agent in agents:
            ...
            if len(agent.options)>0:
                ...
                if agent == leader:
                    print(f"[CAMON] Leader {agent.id}: generating new team plan")
                    generate_plan(agent, global_data)
                else:
                    print(f"[CAMON] Agent {agent.id}: proposing new action to leader {leader.id}")
        ...
        for agent in agents:
            # If agent is inside helicopter, force that option.
            if agent.type == 0 and agent.extra_variables[2] == 1:
                agent.options = [Action(type=0, param_1=0, param_2=0,description="ride helicopter")]
                agent.action_queue = []

            # Optionally review feasibility before issuing the next primitive.
            if cfg.enable_option_review:
                review_option_feasibility(agent, global_data, horizon=2)

            if getattr(agent, "force_stop_next_step", False):
                print(f"[CAMON] Agent {agent.id}: forcing stop (no-op) this step.")
                env_action[agent.id] = [0, 0, 0]
                agent.force_stop_next_step = False
                continue

            if len(agent.options) == 0:
                print(f"[CAMON] Agent {agent.id}: no option to execute this step (idle).")
                env_action[agent.id] = [0, 0, 0]
                continue

            agent.log_chat("Executing Actions", [("system", agent.options[0])])
            print(f"[CAMON] Agent {agent.id}: executing option -> {agent.options[0].description}")

            action_array = generate_action_from_option(agent=agent)
            env_action[agent.id] = action_array if action_array is not None else [0, 0, 0]
            print(f"[CAMON] Agent {agent.id}: low-level action {env_action[agent.id]}")
            agent.log_chat("", [("system", f"{t}: {env_action[agent.id]}")])

        print("[CAMON] Env action tensor (per-agent):", env_action)
