...
class Config:
    ...
    """Whether to enable per-agent feasibility review of planned actions."""
...
def wildfire_alg(cfg: Config):
    ...
    from crew_algorithms.wildfire_alg.algorithms.HMAS_2.utils import (
        propose_actions,
        provide_feedback,
        translate_action,
    ...
    for t in range(cfg.envs.max_steps):
        ...
        for agent in agents:
            ...
            # Optional per-agent feasibility review of the high-level action
            if cfg.enable_option_review:
                should_execute = review_planned_action(agent, action_str, global_data, horizon=1)
                if not should_execute:
                    print(f"[HMAS-2] Agent {agent.id}: cancelling planned action and idling this step.")
                    env_action[agent.id] = [0, 0, 0]
                    global_data["step_history"][f"time: {t}"].update({
                        f"AGENT_{agent.id}": {
                            "state": agent.last_position,
                            "action": f"CANCELLED: {agent.last_cancelled_option}",
                        }
                    })
                    continue

            # If a forced stop was requested (e.g., from a previous review), send a no-op
            if getattr(agent, "force_stop_next_step", False):
                print(f"[HMAS-2] Agent {agent.id}: forcing stop (no-op) this step.")
                env_action[agent.id] = [0, 0, 0]
                agent.force_stop_next_step = False
                global_data["step_history"][f"time: {t}"].update({
                    f"AGENT_{agent.id}": {
                        "state": agent.last_position,
                        "action": f"FORCED_STOP: {action_str}",
                    }
                })
                continue
