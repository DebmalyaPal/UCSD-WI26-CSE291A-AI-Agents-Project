...
BASE_DIR   = 'crew_algorithms/wildfire_alg'
        ...
        for algo in data.get(level, {}):
            for df in data[level][algo]:
                if 'cumulative_score' in df:
                    cut_trees_max = max(cut_trees_max, df['cumulative_score'].max())
        return cut_trees_max
...
def export_bcs_stats(data):
    ...
    for lvl in levels:
        ...
        for seed in all_seeds:
            # Baseline: DO-NOTHING
            do_nothing_dfs = data[lvl].get('MANUAL', [])
            baseline = None
